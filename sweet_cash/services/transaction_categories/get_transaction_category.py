...
class GetTransactionCategory(BaseService):
    def __init__(self,
                 user_id: int,
    ...
    async def __call__(self, transaction_category_id) -> TransactionCategoryModel:
        async with self.transaction_categories_repository.transaction():
            result: TransactionCategoryModel = await self.transaction_categories_repository.get_transaction_category_by_id(transaction_category_id)
        return result
