...
class SendPasswordChangeEmail(object):
    ...
    async def __call__(self, email: str) -> None:
        ...
        try:
            ...
            msg['To'] = email
            msg['Subject'] = 'Смена пароля в Sweet Cash'
            ...
            content = f"""\
                    <html>
                      <body>
                        <p>Привет!<br>
                           Для смены пароля в Sweet Cash перейдите по
                           <a href="{Settings.HOST}/changePassword?email={email}&code={confirmation_code}">ссылке</a>.
                        </p>
                      </body>
                    </html>
                    """
        ...
        logger.info(f'Email for password change sent to address {email}')
