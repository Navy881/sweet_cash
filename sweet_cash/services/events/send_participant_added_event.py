...
from sweet_cash.types.notifications_events import EventType, ParticipantsAddedData
...
class SendParticipantAddedEvent(BaseService):
    def __init__(self,
                 user_id: int,
    ...
    async def __call__(self, event_id: int, user_id: int, role: EventParticipantRole) -> None:
        ...
        for participant in events_participants:
            await self.send_event(event_type=EventType.PARTICIPANT_ADDED.value,
