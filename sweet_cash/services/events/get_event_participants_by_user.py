...
class GetEventParticipantsByUser(BaseService):
    ...
    async def __call__(self, user_id: int, accepted: bool = True) -> List[EventsParticipantsModel]:
        ...
        async with self.events_participants_repository.transaction():
            event_participants: List[EventsParticipantsModel] = await self.events_participants_repository. \
                get_events_participants_by_user(user_id=user_id, accepted=accepted)
