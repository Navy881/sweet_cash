...
class GetEventsByRole(BaseService):
    def __init__(self,
                 ...
                 user_partisipants: UsersRepository) -> None:
        self.user_id = user_id
        self.events_repository = events_repository
        self.events_participants_repository = events_participants_repository
        self.user_partisipants = user_partisipants
    ...
    async def __call__(self, roles: str) -> List[EventModel]:
        ...
        async with self.user_partisipants.transaction():
            for i, events_participant in enumerate(events_participants):
                user = await self.user_partisipants.get_by_id(events_participant.user_id)
